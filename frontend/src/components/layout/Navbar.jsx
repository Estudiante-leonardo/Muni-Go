...
export default function Navbar({ setIsMenuOpen }) {
  ...
  const handleDropdownKeyDown = (e) => {
    if (!isDropdownOpen) {
      if (e.key === 'Enter' || e.key === ' ' || e.key === 'ArrowDown') {
        ...
        const idx = municipalidades.findIndex(m => m.id === selectedMunicipalidadId);
  ...
  const selectedMuni = municipalidades.find(m => m.id === selectedMunicipalidadId);
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            >
              ...
              <svg className={`w-4 h-4 ml-1 flex-shrink-0 text-slate-500 transition-transform ${isDropdownOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
